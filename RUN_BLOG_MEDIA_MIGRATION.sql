-- Complete migration script for blog_media table
-- Run this in Supabase SQL Editor
-- Make sure to create the 'blog-media' storage bucket first via Dashboard
...
-- Note: Using (select auth.uid()) directly in policies to cache auth.uid() per query
...
CREATE INDEX IF NOT EXISTS idx_blog_media_blog_post_id ON public.blog_media(blog_post_id);
CREATE INDEX IF NOT EXISTS idx_blog_media_media_type ON public.blog_media(media_type);
CREATE INDEX IF NOT EXISTS idx_blog_media_display_order ON public.blog_media(blog_post_id, display_order);
